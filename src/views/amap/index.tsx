...
export default () => {
  ...
  // 移除指定点标记
  const removeAMapMarker = (mk: any) => {
    mapRef.remove(mk);
    setcurrentMarkersArray((currentMarkersArray = []));
  };
  // 画圆
  ...
  // 添加信息窗口
  const addAMapInfoWindow = (infoWindowOpts: any) =>
    new AMap.InfoWindow(infoWindowOpts);
  // 移除信息窗口
  const removeAMapInfoWindow = () => {
    mapRef.clearInfoWindow();
  };
  // 返回地图中心点（当前定位点）
  ...
  return (
    <div className="amap-wrapper">
      ...
      <div
        title="点击聚焦当前点"
        className="backCenter-AMap"
        onClick={backToMapCenter}
      />
    </div>
  );
};
