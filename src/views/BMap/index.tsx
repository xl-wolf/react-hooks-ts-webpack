...
export default () => {
  ...
  let [currentPositionPointsArray, setcurrentPositionPointsArray] = useState([
    118.10388605,
    24.48923061,
  ]);
  ...
  const enableDefaultLoc = (point: any) => {
    ...
    const iconConfig = {
      url: require("./images/red.png").default,
    ...
    mk.addEventListener("dragend", function (e: any) {
      ...
      // 移除自身以外的其他点标记
      allMarkers.forEach(
        (marker: any) => marker !== this && removeBMapMarker(marker)
      );
      // 随机生成当前位置旁边的N个点标记
  ...
  const generateNearByNMarkers = (e: any, count: number = 20) => {
    for (let i = 0; i < count; i++) {
      ...
      const iconConfig = {
        url: require("./images/blue.png").default,
  ...
  const drawCircle = (
    ...
    const BmapCircle = new BMap.Circle(
      currentPositionPointsObj,
      radius,
      circleConfig
    );
